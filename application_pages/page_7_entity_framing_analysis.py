...
def main():
    st.markdown(
        "### Story: Bias Detection: Entity Representation and Framing Analysis")
    st.markdown(
        """
    ...
    if 'target_entities_multiselect' not in st.session_state:
        st.session_state.target_entities_multiselect = [
            "GlobalBank Inc.", "TechInnovate Solutions", "GreenEnergy Corp."]

    target_entities_input = st.multiselect(
        "Target Entities for Analysis (Optional)",
        options=["GlobalBank Inc.", "TechInnovate Solutions",
                 "GreenEnergy Corp.", "CEO", "CFO", "analyst"],
    ...
    if st.button("Analyze Entity Framing Bias", key="analyze_entity_btn"):
        with st.spinner("Analyzing entity framing bias..."):
            st.session_state.entity_framing_analysis_df, fig = analyze_entity_framing_bias(
                st.session_state.data, target_entities=target_entities_input)
    ...
    # Show results if entity framing analysis has been performed
    if 'entity_framing_analysis_df' in st.session_state:
        ...
        st.markdown("---")
        if st.button("Next: Go to Adversarial Testing", key="next_page_7", type="primary"):
