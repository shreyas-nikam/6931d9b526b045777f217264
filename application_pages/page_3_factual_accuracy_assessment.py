...
def main():
    st.markdown("### Story: Hallucination Assessment: Factual Accuracy via Key Fact Extraction")
    st.markdown(
        """
    ...
    st.markdown(r"$$ Factual\_Accuracy = \frac{\text{Number of Correctly Extracted Facts}}{\text{Total Number of Ground Truth Facts}} $$")
    st.markdown(
        """
    ...
    if st.session_state.current_step >= 3:
        ...
        st.markdown(f"Your current overall factual accuracy score is **{avg_accuracy:.2f}**. As a Risk Manager, this score directly indicates the reliability of the LLM's financial reporting. A score below $Y\%$ necessitates human review to prevent misinformed investment decisions.")

        st.markdown("---")
        st.subheader("Detailed Fact Review per Document")
        if 'fact_doc_selector' not in st.session_state:
            st.session_state.fact_doc_selector = list(st.session_state.data.keys())[0]
        ...
        selected_doc_id_fact = st.selectbox(
            ...
            index=list(st.session_state.data.keys()).index(st.session_state.fact_doc_selector) if st.session_state.fact_doc_selector in st.session_state.data else 0
        )
        ...
        if selected_doc_id_fact and 'fact_assessment' in st.session_state.data[selected_doc_id_fact]:
            ...
            st.markdown(f"**Document: {selected_doc_id_fact} - {doc_data['company']}**")
            st.dataframe(pd.DataFrame(doc_data['fact_assessment']['detailed_results']))
            st.markdown(f"**Correct Facts:** {doc_data['fact_assessment']['correct_facts']}")
            st.markdown(f"**Missing Facts:** {doc_data['fact_assessment']['missing_facts']}")
            st.markdown(f"**Hallucinated Financial Values (potential):** {doc_data['fact_assessment']['hallucinated_facts']}")
            st.markdown(f"**Factual Accuracy Score for this Document:** {doc_data['fact_assessment']['accuracy_score']:.2f}")
