...
def main():
    st.markdown("### Story: Prompt Engineering Strategy: Adversarial Testing for Emergent Risks")
    st.markdown(
        """
    ...
    if 'adversarial_doc_selector' not in st.session_state:
        st.session_state.adversarial_doc_selector = 'doc_002' if 'doc_002' in st.session_state.data else (list(st.session_state.data.keys())[0] if st.session_state.data else None)
    ...
    if st.button("Run Adversarial Tests", key="run_adv_btn"):
        if adv_doc_id and adv_doc_id in st.session_state.data:
            with st.spinner(f"Running adversarial tests on {adv_doc_id}..."):
                ...
                st.session_state.current_step = 8 # Move to next logical step
                st.rerun()
        else:
